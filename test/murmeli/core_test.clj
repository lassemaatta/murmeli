(ns murmeli.core-test
  (:require [clojure.spec.test.alpha :as stest]
            ...
            [murmeli.operators :refer [$addFields $eq $exists $group $gt $gte $jsonSchema $listLocalSessions $lt
                                       $match $project $push $set $sum]]
            [murmeli.specs]
...
(deftest missing-codec-test
  (let [^CodecRegistry registry (m/registry {})]
        ...
        (m/insert-many! conn
                        ...
                        [{:university "USAL"
                         ...
                         {:university "USAL"
                          :name       "Communication"
                          :level      "Excellent"}]))
      (is (reducible? (m/aggregate-reducible-db! conn [])))
      (testing "$match"
        (is (match? (matchers/seq-of {:_id     {:id  binary?
                                                :uid binary?}
                                      :lastUse inst?})
                    (m/aggregate-db! conn [{$listLocalSessions {:allUsers true}}]))))
      (testing "$project"
        (is (match? (matchers/seq-of {:lastUse inst?})
                    (m/aggregate-db! conn [{$listLocalSessions {:allUsers true}}
                                           {$project {:_id 0}}])))))))
